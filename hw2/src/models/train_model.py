...
def train(cfg: DictConfig):
    ...
    # 1. Читаем данные с диска (теперь пайплайн зависит от файла)
    df = pd.read_csv(cfg.dataset.raw_data_path)
    ...
    model = get_model(cfg)
    model.fit(X_train, y_train)
    log.info(f"Model trained: {cfg.model.name}")
    ...
    # 4. Сохранение модели и метрик (для DVC)
    with open(cfg.dataset.output_model_path, 'wb') as f:
        pickle.dump(model, f)

    # Сохраним метрики в json, чтобы DVC мог их показать
    metrics = {"accuracy": accuracy}
    with open(cfg.dataset.metrics_path, 'w') as f:
        json.dump(metrics, f)
